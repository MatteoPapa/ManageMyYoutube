class AffiliationsController  #< ApplicationController #questo da problemi in fase di test
  #before_action :set_affiliation, only: %i[ show edit update destroy ]

  # GET /affiliations or /affiliations.json
  def self.prova(input)
    0
  end
 ...
 # GET /affiliations/accept?id=...
  ...
  def self.createaff(cliente1,manager1,azienda1)
    @affiliation = Affiliation.new(cliente1,manager1,azienda1)

  
      if @affiliation.save
        "affiliazione salvata"
